...
from utils.knowledge_base import get_analysis_documents, get_resume_documents, get_role_guide_documents
...
def _init_state():
    defaults = {
        ...
        "chat_messages": [],       # session memory for the chat tab
        "vector_store": None,      # RAG index, rebuilt as new data becomes available
        "vector_store_signature": None,  # tracks what data the index was built from
    }
...
with st.sidebar:
    ...
    groq_model_choice = st.selectbox(
        "Groq Model",
        options=[GROQ_MODEL_PRIMARY, GROQ_MODEL_FAST, GROQ_MODEL_ALT],
        index=0,
        help=(
            "Groq periodically retires older models -- if you get a "
            "'model not found' error, try a different option here. "
            "gpt-oss-120b = best quality + tool calling, gpt-oss-20b = "
            "fastest/cheapest, qwen3.6-27b = alternate."
        ),
    )
...
def ensure_vector_store() -> VectorStore:
    """
    Build (or rebuild, if the underlying data changed) the RAG vector store
    from the static role-guide knowledge base plus whatever resume/analysis
    data is currently available in session state. Cached in session_state
    so we don't re-chunk/re-embed on every single Streamlit rerun.
    """
    ...
    if st.session_state.get("vector_store") is not None and st.session_state.get("vector_store_signature") == signature:
        return st.session_state["vector_store"]

    documents = get_role_guide_documents()
    if profile:
        documents += get_resume_documents(profile, st.session_state.get("resume_text", ""))
    if gap_result:
        documents += get_analysis_documents(gap_result, roadmap or [], target_role)

    vs = VectorStore(chunk_size=500, chunk_overlap=80)
    vs.add_documents(documents)

    st.session_state["vector_store"] = vs
    st.session_state["vector_store_signature"] = signature
    return vs
...
with tab1:
    ...
    with analyze_col1:
        run_resume_analysis = st.button("🔍 Analyze Resume", type="primary", use_container_width=True)
...
# ==========================================================================
# TAB 5 — AI Career Chat (RAG + Tool Calling + Session Memory)
# ==========================================================================
with tab5:
    st.subheader("💬 Chat with Aria, Your AI Career Coach")
    st.caption(
        "This chat is grounded with **Retrieval-Augmented Generation** (role guides + your resume + "
        "your results are chunked, embedded, and retrieved) and can call **live tools** "
        "(skill-gap lookup, roadmap lookup, interview-question search) — the model decides for itself "
        "when to use them. Full conversation history is remembered for this session."
    )

    vector_store = ensure_vector_store()
    vs_stats = vector_store.stats()
    st.caption(
        f"📚 Knowledge base indexed: **{vs_stats['num_chunks']} chunks** "
        f"(chunk size {vs_stats['chunk_size']}, overlap {vs_stats['chunk_overlap']}) — "
        f"embedding backend: `{vs_stats['backend']}`"
    )

    if not st.session_state.get("gap_result"):
        st.info(
            "💡 Tip: run **Resume Analysis** (Tab 1) and **Skill Gap Analysis** (Tab 2) first so Aria "
            "can ground answers in your actual results — but you can also just chat about general "
            f"career guidance for **{target_role}** right now."
        )
    ...
    for msg in st.session_state["chat_messages"]:
        with st.chat_message(msg["role"]):
            ...
            if msg.get("tool_log"):
                with st.expander(f"🔧 {len(msg['tool_log'])} tool call(s) used to answer this"):
                    for call in msg["tool_log"]:
                        st.markdown(f"**Tool:** `{call['tool']}`  \n**Input:** `{call['input']}`")
                        st.caption(call["result"][:400] + ("..." if len(call["result"]) > 400 else ""))

    user_query = st.chat_input(f"Ask Aria anything about your {target_role} readiness...")
    ...
    if user_query:
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤖 Aria is thinking (retrieving context, deciding on tools)..."):
                try:
                    llm_client = get_llm_client()
                    chat_agent = ChatAgent(llm_client)
                    # Pass the FULL history (excluding tool_log metadata) for session memory.
                    history_for_llm = [
                        {"role": m["role"], "content": m["content"]}
                        for m in st.session_state["chat_messages"]
                    ]
                    answer, tool_log = chat_agent.respond(
                        chat_history=history_for_llm,
                        target_role=target_role,
                        gap_result=st.session_state.get("gap_result") or {},
                        roadmap=st.session_state.get("roadmap") or [],
                        interview_questions=st.session_state.get("interview_questions") or [],
                        vector_store=vector_store,
                    )
                    st.markdown(answer)
                    if tool_log:
                        with st.expander(f"🔧 {len(tool_log)} tool call(s) used to answer this"):
...
st.divider()
st.caption(
    "Built with a 5-agent architecture (Resume Parser → Gap Analyzer → Roadmap Agent → Interview Agent "
    "→ RAG+Tool-Calling Chat Agent) orchestrated via LangGraph, powered by Anthropic Claude / Groq Llama. "
    "The chat tab uses real Retrieval-Augmented Generation (chunking + embeddings + vector search) and "
    "native LLM tool-calling with persistent session memory. "
